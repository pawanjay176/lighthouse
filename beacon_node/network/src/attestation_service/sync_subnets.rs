...
pub struct SyncCommitteeService<T: BeaconChainTypes> {
    ...
    /// The collection of all currently subscribed subnets (random **and** from validator subscriptions).
...
impl<T: BeaconChainTypes> SyncCommitteeService<T> {
    ...
    fn add_known_validator(&mut self, validator_index: u64) {
        if self.known_validators.get(&validator_index).is_none() && !self.subscribe_all_subnets {
            ...
            if self.random_subnets.len() < spec.sync_committee_subnet_count as usize {
    ...
    fn subscribe_to_random_subnets(&mut self, no_subnets_to_subscribe: usize) {
        let subnet_count = self.beacon_chain.spec.sync_committee_subnet_count;
    ...
    fn subscribe_to_subnet(&mut self, exact_subnet: ExactSubnet) -> Result<(), &'static str> {
        ...
        self.handle_subscriptions(exact_subnet.clone(), expected_end_subscription_duration);
        ...
        self.unsubscriptions.retain(|subnet| {
            !(subnet.subnet_id == exact_subnet.subnet_id
                && subnet.until_epoch <= exact_subnet.until_epoch)
        });
    ...
    fn handle_subscriptions(
        &mut self,
        exact_subnet: ExactSubnet,
        expected_end_subscription_duration: Duration,
    ) {
        ...
        if let Some(expiry) = self.random_subnets.get(&exact_subnet.subnet_id) {
            ...
            if !self.subscriptions.contains(&exact_subnet.subnet_id) {
                ...
                debug!(self.log, "Subscribing to subnet"; "subnet" => *exact_subnet.subnet_id, "until_epoc" => ?exact_subnet.until_epoch);
                self.subscriptions.insert(exact_subnet.subnet_id);
                self.events
                    .push_back(SubnetServiceMessage::Subscribe(exact_subnet.subnet_id));

                // add the subnet to the ENR bitfield
                // Note: unlike attestation subnets, normal validator subscriptions also reflect in the
                // ENR. Hence, we need to update the ENR bitfield to add the subnet for subscription
                // events as well.
                self.events
    ...
    fn handle_unsubscriptions(&mut self, exact_subnet: ExactSubnet) {
        ...
        // Note: unlike attestation subnets, normal validator subscriptions also reflect in the
        // ENR. Hence, we need to update the ENR bitfield to remove the subnet for unsubscription
        // events as well.
        self.events
    ...
    fn handle_known_validator_expiry(&mut self) {
        let spec = &self.beacon_chain.spec;
        let subnet_count = spec.sync_committee_subnet_count;
