...
pub fn validate_blob<T: EthSpec>(
    ...
    let res1 = kzg.verify_blob_kzg_proof(
        &ssz_blob_to_crypto_blob::<T>(blob.clone())?.as_ref(),
        kzg_commitment,
        kzg_proof,
    )?;

    let res2 = kzg.verify_blob_kzg_proof2(
        MainnetBlob::from_bytes(blob.to_vec().as_slice()).unwrap().as_ref(),
        kzg_commitment,
        kzg_proof,
    )?;
...
pub fn blob_to_kzg_commitment<T: EthSpec>(
    ...
    kzg.blob_to_kzg_commitment(ssz_blob_to_crypto_blob::<T>(blob)?.as_ref())
}
...
pub fn compute_kzg_proof<T: EthSpec>(
    ...
    let z = z.0.into();
    kzg.compute_kzg_proof(ssz_blob_to_crypto_blob::<T>(blob)?.as_ref(), z)
