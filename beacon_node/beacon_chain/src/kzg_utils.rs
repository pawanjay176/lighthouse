...
fn ssz_blob_to_crypto_blob<T: EthSpec>(
    blob: Blob<T>,
    kzg: &Kzg
) -> Result<kzg::Blob, KzgError> {
    kzg.blob_from_bytes(blob.to_vec().as_slice())
}
...
pub fn validate_blobs<T: EthSpec>(
    ...
    let blobs = blobs
        .iter()
        .map(|blob| ssz_blob_to_crypto_blob::<T>(blob.clone(), kzg)) // Avoid this clone
...
pub fn compute_blob_kzg_proof<T: EthSpec>(
    ...
    kzg.compute_blob_kzg_proof(ssz_blob_to_crypto_blob::<T>(blob.clone(), kzg)?, kzg_commitment)
}
